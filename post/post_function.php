...
function createPost($request_values)
	{
		global $connect, $errors, $title, $featured_image, $topic_id, $body;
		...
		$post_check_query = "SELECT * FROM posts WHERE title='$post_title' LIMIT 1";
		$result = mysqli_query($connect, $post_check_query);
		...
		if (count($errors) == 0) {
			...
			if(mysqli_query($connect, $query)){ // if post created successfully
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $connect, $title, $post_slug, $body, $published, $isEditingPost, $post_id;
